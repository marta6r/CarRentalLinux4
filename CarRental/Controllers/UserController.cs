...
namespace CarRental.Controllers
{
    public class UserController : Controller
    {
        ...
        // Список автомобилей (с сортировкой)
        public async Task<IActionResult> Index()
        {
            var cars = await _context.Cars
                .Include(c => c.Category)
                .Where(c => c.IsAvailable)
                .OrderBy(c => c.Brand)      // Сортировка по марке (А-Я)
                .ThenBy(c => c.Model)       // Затем по модели (А-Я)
                .ToListAsync();
        ...
        public async Task<IActionResult> AISearch([FromBody] JsonElement request)
        {
            ...
            try
            {
                ...
                var recommendedCars = cars
                    .Where(c => recommendedIds.Contains(c.Id))
                    .OrderBy(c => c.Brand)      // Сортировка рекомендованных авто
                    .ThenBy(c => c.Model)
                    .ToList();
